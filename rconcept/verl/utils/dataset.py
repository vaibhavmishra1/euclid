...
import json
import random
def _get_active_concepts_from_env() -> str:
    """
    Read a simple env var set by our concept-curriculum runner.
    ...
    Returns a human-readable string (or empty string if unset).
    """
    raw = os.getenv("RZERO_ACTIVE_CONCEPTS", "").strip()
    if not raw:
        return ""
...
class RLHFDataset(Dataset):
    ...
    def _build_messages(self, example: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        active_concepts = _get_active_concepts_from_env()
        concept_clause = ""
        if active_concepts:
            concept_clause = (
                "\n\nCONCEPT_CONSTRAINT:\n"
                f"- Your problem MUST meaningfully use ALL of these concepts: {active_concepts}\n"
                "- Do not merely mention them; the solution must require them.\n"
            )
        if "questioner_format_with_persona" in self.format_prompt:
            print("load personas")
            return [
